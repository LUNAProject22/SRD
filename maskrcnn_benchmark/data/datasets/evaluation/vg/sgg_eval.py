...
class SGRecall(SceneGraphEvaluation):
    ...
    def calculate_recall(self, global_container, local_container, mode, ds_name="vg"):
        ...
        if 'ag' in ds_name:
            pred_rels = np.column_stack((pred_classes[pred_rel_inds], rel_scores.argmax(1)))
            pred_scores = rel_scores.max(1)
        else:
            pred_rels = np.column_stack((pred_rel_inds, 1+rel_scores[:,1:].argmax(1)))
            pred_scores = rel_scores[:,1:].max(1)

        gt_triplets, gt_triplet_boxes, _ = _triplet(gt_rels, gt_classes, gt_boxes, ds_name=ds_name)
        local_container['gt_triplets'] = gt_triplets
        local_container['gt_triplet_boxes'] = gt_triplet_boxes

        pred_triplets, pred_triplet_boxes, pred_triplet_scores = _triplet(
                pred_rels, pred_classes, pred_boxes, pred_scores, obj_scores, ds_name)
...
class SGNoGraphConstraintRecall(SceneGraphEvaluation):
    ...
    def calculate_recall(self, global_container, local_container, mode, ds_name="vg"):
        ...
        if 'ag' in ds_name:
            ...
            nogc_pred_rels = np.column_stack((pred_classes[pred_rel_inds[nogc_score_inds[:,0]]], nogc_score_inds[:,1]))
        ...
        nogc_pred_triplets, nogc_pred_triplet_boxes, _ = _triplet(
                nogc_pred_rels, pred_classes, pred_boxes, nogc_pred_scores, obj_scores, ds_name
        )
...
class SGZeroShotRecall(SceneGraphEvaluation):
    ...
    def prepare_zeroshot(self, global_container, local_container, ds_name):
        gt_rels = local_container['gt_rels']
        gt_classes = local_container['gt_classes']
...
def _triplet(relations, classes, boxes, predicate_scores=None, class_scores=None, ds_name='ag'):
    """
    ...
    if 'ag' in ds_name:
        ...
        ob_idx = []
        p_label = []
        for head, tail, pred in zip(sub_id, ob_id, pred_label):
            # filter out triplets that are not annotated. (sbj/obj not found in gt_class)
            if (head not in classes) or (tail not in classes):
                continue
            sub_idx.append(np.where(classes == head)[0][0])
            ob_idx.append(np.where(classes == tail)[0][0])
            p_label.append(pred)
        triplets = np.column_stack((classes[sub_idx], np.array(p_label), classes[ob_idx]))
        triplet_boxes = np.column_stack((boxes[sub_idx], boxes[ob_idx]))
    else:
    ...
    if predicate_scores is not None and class_scores is not None:
        if 'ag' in ds_name:
            triplet_scores = np.column_stack((class_scores[sub_idx], predicate_scores, class_scores[ob_idx]))
        else:
