...
from maskrcnn_benchmark.data.datasets.evaluation.vg.sgg_eval import SGRecall, SGNoGraphConstraintRecall, SGZeroShotRecall, \
    SGNGZeroShotRecall, SGPairAccuracy, SGMeanRecall, SGNGMeanRecall, SGAccumulateRecall
...
def do_vg_evaluation(
    ...
    if not cfg.MODEL.RELATION_ON: # train faster-rcnn
        mode = 'preddet'
    elif cfg.MODEL.ROI_RELATION_HEAD.USE_GT_BOX:
    ...
    if "bbox" in iou_types and mode == 'sgdet':
        ...
        else: # use obj_classes instead
            fauxcoco.dataset = {
                'info': {'description': 'use coco script for ag detection evaluation'},
                'images': [{'id': i} for i in range(len(groundtruths))],
                'categories': [
                    {'supercategory': 'person', 'id': i, 'name': name}
                    for i, name in enumerate(dataset.obj_classes) if name != '__background__'
                    ],
                'annotations': anns,
            }
    ...
    if "relations" in iou_types:
        ...
        if hasattr(dataset, 'ind_to_predicates'):
            ...
            prd_classes = ['__background__'] + dataset.prd_classes
            eval_mean_recall = SGMeanRecall(result_dict, num_rel_category, prd_classes, print_detail=True)
        ...
        if hasattr(dataset, 'ind_to_predicates'):
            ...
            prd_classes = ['__background__'] + dataset.prd_classes
            eval_ng_mean_recall = SGNGMeanRecall(result_dict, num_rel_category, prd_classes, print_detail=True)
    ...
    if output_folder:
        ...
        for image_id, (groundtruth, prediction) in enumerate(zip(groundtruths, predictions)):
            ...
            if hasattr(dataset, 'categories'):
                ...
                groundtruth = [
                    [b[0], b[1], b[2], b[3], dataset.obj_classes[l]] # xyxy, str
                    for b, l in zip(groundtruth.bbox.tolist(), groundtruth.get_field('labels').tolist())
                    ]
                prediction = [
                    [b[0], b[1], b[2], b[3], dataset.obj_classes[l]] # xyxy, str
...
def evaluate_relation_of_one_image(groundtruth, prediction, global_container, evaluator, ds_name):
    ...
    # evaluator['eval_ng_zeroshot_recall'].calculate_recall(global_container, local_container, mode, ds_name)
...
if __name__=="__main__":
    result_file = "checkpoints/vctree/vctree-precls-rwt-N25-f15o1/inference/VG_stanford_filtered_with_attribute_test/result_dict.pytorch"
    result_dict = torch.load(result_file)
    
    mode = 'predcls'
    dict_file = 'datasets/vg/VG-SGG-dicts-with-attri.json'
    info = json.load(open(dict_file, 'r'))
    predicate_to_ind = info['predicate_to_idx']
    rel_name_list = sorted(predicate_to_ind, key=lambda k: predicate_to_ind[k])
    ...
    for k in [20, 50]:
        ...
        for n, r in zip(rel_name_list, result_dict[mode + '_mean_recall_list'][k]):
