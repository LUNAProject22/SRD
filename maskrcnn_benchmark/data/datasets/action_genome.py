...
import maskrcnn_benchmark.utils.env as envu
envu.set_up_matplotlib()
# COCO API
from pycocotools import mask as COCOmask
from pycocotools.coco import COCO

import maskrcnn_benchmark.utils.boxes as box_utils
import maskrcnn_benchmark.utils.boxes_rel as box_utils_rel
from maskrcnn_benchmark.structures.bounding_box import BoxList
# from maskrcnn_benchmark.config import cfg
from .ag_config import cfg
from maskrcnn_benchmark.config.paths_catalog import DatasetCatalog

ANN_FN = DatasetCatalog.ANN_FN
ANN_FN2 = DatasetCatalog.ANN_FN2
ANN_FN3 = DatasetCatalog.ANN_FN3
ANN_FN4 = DatasetCatalog.ANN_FN4
ANN_FN5 = DatasetCatalog.ANN_FN5
DATASETS = DatasetCatalog.DATASETS
IM_DIR = DatasetCatalog.IM_DIR
IM_PREFIX = DatasetCatalog.IM_PREFIX


def load_vg_classes(dict_file, add_bg=True):
    """
    ...
    return ind_to_classes, ind_to_predicates, class_to_ind, predicate_to_ind

class JsonDatasetRel(object):
    """A class representing a COCO json dataset."""

    def __init__(self, name, transforms=None, flip_aug=False):
        ...
        if os.path.exists(cache_filepath):
            with open(cache_filepath, 'rb') as f:
                self.roidb = pickle.load(f)
                f.close()
        else:
            self.filenames = []
            self.roidb = self.get_roidb(gt=True)
        filenames_cache_path = os.path.join(self.cache_path, self.name + '_filenames.pkl')
        if os.path.exists(filenames_cache_path):
            with open(filenames_cache_path, 'rb') as f:
                self.filenames = pickle.load(f)
                f.close()
        self.categories = {i : self.obj_classes[i] for i in range(len(self.obj_classes))} # ag classes
        # load vg classes
        # self.vg_ind_to_classes, self.vg_ind_to_predicates, self.vg_class_to_ind, self.vg_predicate_to_ind = \
        #     load_vg_classes('datasets/vg/VG-SGG-dicts.json')
        # self.categories = {i : self.vg_ind_to_classes[i] for i in range(len(self.vg_ind_to_classes))} # vg classes
        # self.name_mapping = json.load(open('datasets/ag/name_mapping.json'))


    def ag_obj_to_vg(self, l: int):
        ag_class = self.obj_classes[int(l)]
        if ag_class in self.name_mapping["obj_ag_to_vg"]:
            vg_class = self.name_mapping["obj_ag_to_vg"][ag_class]
            if vg_class in self.vg_class_to_ind:
                l_vg = self.vg_class_to_ind[vg_class]
            else:# cannot find this ag class in vg dict, regard it as __background__
                l_vg = 0
        else: # overlapping class of ag and vg
            l_vg = self.vg_class_to_ind[ag_class]
        # print('AG obj {}:{} ==> VG obj {}:{}'.format(l, ag_class, l_vg, self.vg_ind_to_classes[l_vg]))
        return l_vg

    def ag_prd_to_vg(self, p: int):
        ag_prd = self.prd_classes[int(p)]
        if ag_prd in self.name_mapping["prd_ag_to_vg"]:
            vg_prd = self.name_mapping["prd_ag_to_vg"][ag_prd]
            if vg_prd in self.vg_predicate_to_ind:
                l_vg = self.vg_predicate_to_ind[vg_prd]
            else:# cannot find this ag class in vg dict, regard it as __background__
                l_vg = 0
        else:
            l_vg = self.vg_predicate_to_ind[ag_prd]
        # print('AG predicate {}:{} ==> VG predicate {}:{}'.format(p, ag_prd, l_vg, self.vg_ind_to_predicates[l_vg]))
        return l_vg


    def ag_to_vg(self, target):
        labels = target.get_field("labels")
        relations = target.get_field("relation")
        vg_labels = []
        for i in range(labels.shape[0]):
            vg_labels.append(self.ag_obj_to_vg(int(labels[i])))
        vg_prd_labels = []
        for j in range(relations.shape[0]):
            triplet = relations[j]
            sbj = self.ag_obj_to_vg(int(triplet[0]))
            obj = self.ag_obj_to_vg(int(triplet[1]))
            prd = self.ag_prd_to_vg(int(triplet[2]))
            vg_triplet = np.array([sbj, obj, prd])
            vg_prd_labels.append(vg_triplet)
        assert(len(vg_labels)==labels.shape[0])
        assert(len(vg_prd_labels)==relations.shape[0])
        vg_labels = np.array(vg_labels)
        vg_prd_labels = np.array(vg_prd_labels)
        target.extra_fields["labels"] = torch.from_numpy(vg_labels)
        target.extra_fields["relation"] = torch.from_numpy(vg_prd_labels)
        return target
    ...
    def get_img_info(self, index):
        roidb_i = self.roidb[index]
        return roidb_i

    def __getitem__(self, index):
        roidb_i = self.roidb[index]
        img = Image.open(roidb_i["image"]).convert("RGB")
        if img.size[0] != roidb_i['width'] or img.size[1] != roidb_i['height']:
            print('='*20, ' ERROR index ', str(index), ' ', str(img.size), ' ', str(roidb_i['width']), ' ', str(roidb_i['height']), ' ', '='*20)

        flip_img = (random.random() > 0.5) and self.flip_aug and ('train' in self.name)
        if flip_img:
    ...
    def get_groundtruth(self, index, evaluation=False, flip_img=False):
        roidb_i = self.get_img_info(index)
        w, h = roidb_i['width'], roidb_i['height']
        box = roidb_i["boxes"]
        ...
        target.add_field("labels", torch.from_numpy(roidb_i["gt_classes"]) - 1) # to exlude '__background__'
        # target.add_field("attributes", None)
        
        # relation = np.column_stack((roidb_i["sbj_gt_classes"], roidb_i["obj_gt_classes"], roidb_i["prd_gt_classes"])) # (num_rel, 3)
        # filter out triplets that sbj/obj not found in "labels" list.
        relation = []
        for head, tail, prd in zip(roidb_i["sbj_gt_classes"], roidb_i["obj_gt_classes"], roidb_i["prd_gt_classes"]):
            if (head not in target.get_field("labels")) or (tail not in target.get_field("labels")):
                continue
            relation.append([head, tail, prd])
        relation = np.array(relation)
        target.add_field("relation", torch.from_numpy(relation), is_triplet=True)
        ...
        # convert ag label to vg label. This is only used when evaluating model trained with VG.
        # target = self.ag_to_vg(target)
        return target

        
    @property
    def cache_path(self):
        cache_path = os.path.abspath(os.path.join(cfg.OUTPUT_DIR, 'cache'))
        if not os.path.exists(cache_path):
            os.makedirs(cache_path)
            print('Created cache data path:', cache_path)
        return cache_path
        ...
        if gt:
            ...
            if os.path.exists(cache_filepath) and not cfg.DEBUG:
                ...
                if not cfg.DEBUG:
                    ...
                    print('Cache ground truth roidb to %s', cache_filepath)
                    with open(filenames_cache_path, 'wb') as fp:
                        pickle.dump(self.filenames, fp, pickle.HIGHEST_PROTOCOL)
                    print('Cache filenames to %s', filenames_cache_path)
    ...
    def _add_proposals_from_file(
        ...
        for i, entry in enumerate(roidb):
            if i % 2500 == 0:
                print(' {:d}/{:d}'.format(i + 1, len(roidb)))
    ...
    def get_statistics(self):
        cache_filepath = os.path.join(self.cache_path, 'ag_train_rel_gt_roidb.pkl')
        if os.path.exists(cache_filepath):
            with open(cache_filepath, 'rb') as f:
                roidb = pickle.load(f)
                f.close()
        else:
            # dataset = JsonDatasetRel('ag_train')
            roidb = self.roidb
        
        num_obj_classes = self.num_obj_classes
        num_rel_classes = self.num_prd_classes
        fg_matrix = np.zeros((num_obj_classes, num_obj_classes, num_rel_classes), dtype=np.int64)
        # bg_matrix = np.zeros((num_obj_classes, num_obj_classes), dtype=np.int64)

        for ex_ind in tqdm(range(len(roidb)), desc="roidb statistics"):
            gt_labels_sbj = roidb[ex_ind]['sbj_gt_classes']
            gt_labels_obj = roidb[ex_ind]['obj_gt_classes']
            gt_labels_prd = roidb[ex_ind]['prd_gt_classes']

            # For the foreground, we'll just look at everything
            for o1, o2, gtr in zip(gt_labels_sbj, gt_labels_obj, gt_labels_prd):
