...
class WRelationLossComputation(object):
    ...
    def __init__(
        ...
        elif self.rel_loss == "ce_lrwt":
            self.rel_criterion_loss = ReweightingCEwithLW(dim=cfg.MODEL.ROI_RELATION_HEAD.NUM_CLASSES)
...
class ReweightingCEwithLW(nn.Module):
    """
    Reweighting CE with learnable weights.
    """
    def __init__(self, reduction="mean", dim=51):
        super(ReweightingCEwithLW, self).__init__()
        self.reduction=reduction
        self.weights = nn.Parameter(torch.randn(dim)).to("cuda") #random numbers from a standard normal distribution
    ...
    def forward(self, input, target):
        ...
        final_target[torch.arange(0, target.size(0)), target.argmax(1)] = 1.
        idxs = (target[:, 0] != 1).nonzero().squeeze()
        # find out relation labels
        rel_labels = target[:, 1:].argmax(1) + 1

        weights = torch.ones_like(target[:, 0])
        weights[idxs] = -self.weights[rel_labels[idxs]]
        target = final_target
        x = F.log_softmax(input, 1)
        loss = torch.sum(- x * target, dim=1)*weights
        if self.reduction == 'none':
