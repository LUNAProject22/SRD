'''
allow choosing prior knowledge (context-independent knowledge)
...
class ContextBasedPredicatePredictor:
    def __init__(self, prior_file='data/KG/preprocessed/alias_filtered_codebase_h5/train_triples_context_essential_info.pkl',
                 prior_knowledge_file='data/kg_inference/prior_knowledge.pkl', knowledge_key='matrix_rht_sim_paraphrase-mpnet-base-v2',
        ...
        self.matrix_prior_knowledge = self.prepare_pred_prior(prior_knowledge_file, knowledge_key)  # previously ht2vector
    ...
    def prepare_pred_prior(self, prior_knowledge_file, knowledge_key):
        # TODO: to make it matrix-based, not dictionary based
        prior = pkl.load(open(prior_knowledge_file, 'rb'))
        m1 = prior[knowledge_key]
        m1_norm_by_predicate = normalize(m1, p=1.0, dim=0)
        return m1_norm_by_predicate
    ...
    def predict(self, input_context, verbose=False, use_best_rel_only=False):
        ...
        if use_best_rel_only:
            ...
            ht2prediction = {}
            for ht, prob in pair2prob.items():
                # v = self.matrix_prior_knowledge.get(ht, self.pred_default_vector)
                v = self.get_prior_knowledge(ht[0], ht[1])
                v2 = prob * v
                ht2prediction[ht] = v2
        return ht2prediction

    def get_prior_knowledge(self, head, tail):
        return self.matrix_prior_knowledge[:, self.label_to_idx[head], self.label_to_idx[tail]].numpy()
...
def evaluate_image(predictor, info, ks):
    context = info[0]
    gt_triples = info[1]
    pred_triples = predictor.prepare_pht_matrix(context, use_best_rel_only=True)
    # evaluate
...
if __name__ == "__main__":
    ...
    for ht, vec in ht2vec.items():
        print(ht)
    a = ht2vec[('sheep', 'sheep')]
    a
