...
class PostProcessor(nn.Module):
    ...
    def forward(self, x, rel_pair_idxs, boxes, ds_name='ag_val'):
        """
        Arguments:
        ...
        for i, (rel_logit, obj_logit, rel_pair_idx, box) in enumerate(zip(
            ...
            if self.use_gt_box:
                if 'ag' in ds_name:
                    ...
                    obj_scores, obj_pred = obj_logit.max(dim=1)
                else:
                    obj_class_prob[:, 0] = 0  # set background score to 0
            ...
            if 'ag' in ds_name:
                ...
                sorted_rel_class_prob, idx = torch.sort(rel_class_prob, dim=1, descending=True)
                top3_rel_class_prob = sorted_rel_class_prob[:, :3]
                sorted_rel_label = idx
                top3_rel_labels = sorted_rel_label[0, :3]
                rel_pair_idx = rel_pair_idx.repeat((3, 1))
                rel_class_prob = rel_class_prob.repeat((3, 1))
                # make 2nd, 3rd scores be largest score
                rel_class_prob[1][top3_rel_labels[0]] = 0
                rel_class_prob[2][top3_rel_labels[0]] = 0
                rel_class_prob[2][top3_rel_labels[1]] = 0
                rel_labels = top3_rel_labels
            else:
