...
class ROIRelationHead(torch.nn.Module):
    ...
    def __init__(self, cfg, in_channels):
        ...
        self.weak_sup = False
        if cfg.WSUPERVISE.METHOD == "RWT_RSP":
            self.weak_sup = True
    ...
    def forward(self, features, proposals, targets=None, logger=None):
        ...
        """
        if self.training:
            ds_name = self.cfg.DATASETS.TRAIN[0]
            ...
            with torch.no_grad():
                if self.cfg.MODEL.ROI_RELATION_HEAD.USE_GT_BOX:
                    if self.weak_sup and self.training:
                        ...
                        proposals, rel_labels, rel_pair_idxs, rel_binarys = self.samp_processor.gtbox_relsample(proposals, targets, ds_name)
                else:
                    if self.weak_sup and self.training:
            ...
            ds_name = self.cfg.DATASETS.TEST[0]
            if self.cfg.USE_LOGITS.ENABLE:
                proposals, rel_labels, rel_pair_idxs, rel_binarys = self.samp_processor.gtbox_relsample(proposals, targets, ds_name)
            else:
                rel_labels, rel_binarys = None, None
                rel_pair_idxs = self.samp_processor.prepare_test_pairs(targets[0].get_field("labels").device, proposals, ds_name)

        if not (not self.training and self.cfg.TEST.DISABLE_ROI_FEATURE):
        ...
        # copy image path from target to proposal
        if targets[0].has_field("image_path"):
            for t, p in zip(targets, proposals):
                p.add_field("image_path", t.get_field("image_path"))

        # final classifier that converts the features into predictions
        # should corresponding to all the functions and layers after the self.context class
        refine_logits, relation_logits, add_losses = \
            self.predictor(proposals, rel_pair_idxs, rel_labels, rel_binarys, roi_features, union_features, logger)

        # for not ranking pairs
        if self.cfg.USE_LOGITS.ENABLE:
            return None, (relation_logits, rel_pair_idxs, rel_labels), {}

        # for test
        if not self.training:
            result = self.post_processor((relation_logits, refine_logits), rel_pair_idxs, proposals, ds_name)
