...
class DatasetCatalog(object):
    ...
    DATASETS = {
        ...
        'ag_train': {
            IM_DIR:
                DATA_DIR + '/ag/frames',
            ANN_FN:
                DATA_DIR + '/ag/annotations/detections_train.json',
            ANN_FN2:
                DATA_DIR + '/ag/annotations/new_annotations_train.json',
            ANN_FN3:
                DATA_DIR + '/ag/annotations/predicates.json',
            ANN_FN4:
                DATA_DIR + '/ag/annotations/train_fname_mapping.json',
            ANN_FN5:
                DATA_DIR + '/ag/annotations/train_fname_list.json',
        },
        'ag_val': {
            IM_DIR:
                DATA_DIR + '/ag/frames',
            ANN_FN:
                DATA_DIR + '/ag/annotations/detections_val.json',
            ANN_FN2:
                DATA_DIR + '/ag/annotations/new_annotations_val.json',
            ANN_FN3:
                DATA_DIR + '/ag/annotations/predicates.json',
            ANN_FN4:
                DATA_DIR + '/ag/annotations/val_fname_mapping.json',
            ANN_FN5:
                DATA_DIR + '/ag/annotations/val_fname_list.json',
        },
        "NEW_RELATION": {
            "img_dir": "vg/VG_100K",
            "roidb_file": "vg/VG-SGG-with-attri.h5",
    ...
    def get(name, cfg):
        if "coco" in name:
            ...
            args["logits_cfg"] = cfg.USE_LOGITS
            args['relation_cfg'] = cfg.DATASETS.NEW_RELATION
            args['wsupervise_cfg'] = cfg.WSUPERVISE
            return dict(
                factory="VGDataset",
                args=args,
            )
        elif "NEW_RELATION" in name:
            data_dir = DatasetCatalog.DATA_DIR
            args = copy.deepcopy(DatasetCatalog.DATASETS[name])
            for k, v in args.items():
                args[k] = os.path.join(data_dir, v)
