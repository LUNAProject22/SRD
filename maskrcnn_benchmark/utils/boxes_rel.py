...
from .boxes import bbox_transform_inv, bbox_overlaps
...
def filter_new_rel_nn(s_bbox, o_bboxes):
    # calculate nearest neighbour
    s_c = np.array([[(s_bbox[2] + s_bbox[0]) / 2, (s_bbox[3] + s_bbox[1]) / 2]])
    o_cx = np.array((o_bboxes[:, 2] + o_bboxes[:, 0]) / 2)
    o_cy = np.array((o_bboxes[:, 3] + o_bboxes[:, 1]) / 2)
    o_c = np.column_stack((o_cx, o_cy))
    so = np.concatenate((s_c, o_c), axis=0)
    nbrs = NearestNeighbors(n_neighbors=2, algorithm='ball_tree').fit(so)
    distances, indices = nbrs.kneighbors(so)
    o_ii = indices[0, 1] - 1 # index 0 is the subject
    return o_ii


def filter_new_rel_overlapping(s_bbox, o_bboxes):
    overlaps = bbox_overlaps(s_bbox, o_bboxes, to_move=0)[0] > 0
    o_ii = torch.nonzero(overlaps, as_tuple=True)[0]
    return o_ii