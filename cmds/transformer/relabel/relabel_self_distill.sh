...
N=25
TOP_K_FREQ=$((50-N))
OUTPUT_DIR="output/relabel-$MODEL_NAME/self-distillation"
mkdir -p $OUTPUT_DIR
...
python maskrcnn_benchmark/data/datasets/vg_new_relation.py \
    ...
    NEW_RELATION.REL_CACHE_PREFIX ${MODEL_NAME}_pretrain_idf_N${N}_selfD \
    ...
    USE_LOGITS.LOGITS_COMB_PRIOR $OUTPUT_DIR/sup_data_with_${MODEL_NAME}_pred_comb_idf_N${N}_CD_selfD.pk \
    NEW_RELATION.USE_CONTEXT True \
    NEW_RELATION.USE_DISTILLATION True \
    NEW_RELATION.TOP_K_FREQ $TOP_K_FREQ \
    NEW_RELATION.KG_DATA_ROOT datasets/kg/self-distillation \
