...
F=5 # 5, 15
TOP_K_FREQ=$((50-N))
OUTPUT_DIR="output/relabel-$MODEL_NAME/N$N-freq"
mkdir -p $OUTPUT_DIR
...
python maskrcnn_benchmark/data/datasets/vg_new_relation.py \
    ...
    NEW_RELATION.REL_CACHE_PREFIX ${MODEL_NAME}_pretrain_tfidf_N${N}F$F \
    ...
    USE_LOGITS.LOGITS_COMB_PRIOR $OUTPUT_DIR/sup_data_with_${MODEL_NAME}_pred_comb_tfidf_N${N}F${F}_CD.pk \
    NEW_RELATION.TOP_K_FREQ $TOP_K_FREQ \
    NEW_RELATION.CLUSTER_TO_KNOWLEDGE triple_prob-context-distill-tfidf-masked_new_rels_F${F}-N${N}.pkl
